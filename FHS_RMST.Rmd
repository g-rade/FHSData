...
last_contact <-read_dta("FHS 2154 - datasets/dr281_vr_survdth_2014_a_1025s.dta") %>% select(IDTYPE, ranid, DATEDTH, LASTCON)
event_data <- read_dta("FHS 2154 - datasets/dr281_vr_soe_2016_a_1073s.dta") %>% 
  select(IDTYPE, ranid, EVENT, DATE)
all_data2 <- left_join(all_data, last_contact, by = c("IDTYPE", "ranid")) 
all_data2 <- all_data2 %>% 
